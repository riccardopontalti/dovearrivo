...
describe('search window rules', () => {
	...
	it('interprets the local day in Europe/Rome, not UTC', () => {
		...
		expect(windowProblem(r, BEFORE)).toMatch(/same local day/);
	});

	it('rejects past dates and, today, departures earlier than now', () => {
		const now = Date.parse('2026-09-26T12:00:00+02:00');
		expect(windowProblem(request(), Date.parse('2026-09-27T09:00:00+02:00'))).toMatch(/past/);
		expect(windowProblem(request(), now)).toMatch(/earlier than now/);
		expect(
